...
router.post('/',  util.isLoggedin, function(req, res){
    ...
    }else{req.body.author = req.user._id;
...
router.get('/:id',function(req, res){
    ...
    const commentError = req.flash('commentError')[0] ||{_id:null, errors:{}};
    Promise.all([
        Post.findOne({_id:req.params.id}).populate({ path: 'author', select: 'username' }),
        Comment.find({post:req.params.id}).sort('createdAt').populate({ path: 'author', select: 'username' })
      ])
      .then(([post, comments]) => {
        res.render('posts/detail', { post:post, comments:comments, commentForm:commentForm, commentError:commentError});
      })
      .catch((err) => {
        console.log('err: ', err);
        return res.json(err);
